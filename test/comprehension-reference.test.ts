...
test('reference state table exactly freezes taper, repair, rediagnosis, and reset rows', () => {
  ...
  assert.match(state, /On rediagnosis, map every candidate only to an explicit prior-answer element[\s\S]*Write each description as that source phrase or a close source paraphrase[\s\S]*never “what someone sends\s+when using a website.”[\s\S]*After a failed diagnosis, lead with a different source\s+proposition and choose at least two materially distinct paths absent from its\s+failed candidate set[\s\S]*Never merely relabel, split, or reorder its old coverage[\s\S]*do not add an unseen actor, success branch, event, or condition[\s\S]*Do not turn `process` into `succeeds` unless the prior answer says so/i);

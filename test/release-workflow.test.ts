...
test('release keeps package.json and the skill version in sync and re-verifies dist', () => {
  assert.match(release, /release:prepare -- --write/);
  assert.match(release, /git add package\.json skill\/im-dumb\/SKILL\.md CHANGELOG\.md/);

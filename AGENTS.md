...
M5 phases 1–2 are shipped: `skill/im-dumb/references/learning-assets.md`, the `learning-asset` Layer 1 structure checker (markdown, HTML, and `slides` formats), and the `learning-asset` golden category. A slide deck is HTML — one `<article>`, at least two `<section class="slide">`, each with its own heading. There is no PowerPoint binary output. AV (phase 3) is still ahead. The SKILL.md body target is back to 900 words: the comprehension non-trigger examples and no-snapshot fallback moved into `references/comprehension.md`, which is only read once repair is in play. The checker's warn threshold stays 1000 (D12 unchanged). Prefer moving prose into `references/` over raising the target — only rules needed on *every* turn belong in the always-loaded body.
